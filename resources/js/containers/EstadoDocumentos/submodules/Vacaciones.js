...
export const Vacaciones = () => {
    ...
    useEffect(() => {
        ...
        function fetchTipoDocumentosTurecibo() {
            ...
            setLoading(true);
            Axios.get(`
                /api/documentos-turecibo?tipo_documento_turecibo_id=${3}&empresa_id=${filter.empresa_id}&estado=${filter.estado}&regimen_id=${filter.regimen_id}&zona_labor_id=${filter.zona_labor_id}
            `)
                .then(res => {
                    console.log(res);
    ...
    return (
        <>
            ...
            <h4>Vacaciones <small>{usuario.estado_documentos === 2 ? '(Administrador)' : ''}</small></h4>
            <br />
            <div className="row">
                <div className="col-md-6 col-sm-12">
                    <BuscarTrabajador
                        tipoDocumento={{ name: "VACACIONES", id: 3}}
                    />
                </div>
